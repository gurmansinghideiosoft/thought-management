...
export function TaskTagManager({ trigger }: { trigger: React.ReactNode }) {
  const { data: tags, isLoading } = useListTaskTagsQuery();
  const [createTag, { isLoading: creating }] = useCreateTaskTagMutation();
  ...
  return (
    <Dialog>
      ...
      <DialogContent title="Task tags" description="Reused across all your tasks.">
        {isLoading ? (
          <CenteredSpinner />
        ) : (
